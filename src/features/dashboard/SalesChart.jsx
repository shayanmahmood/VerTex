...
function SalesChart({ shares, numDays }) {
  ...
  const data = allDates.map((date) => {
    return {
      ...
      totalSales: shares?.filter((booking) => isSameDay(date, new Date(booking.created_at))).reduce((acc, cur) => acc + cur.stocks.perStockPrice * cur.numShare, 0),
      sharesSold: shares?.filter((booking) => isSameDay(date, new Date(booking.created_at))).reduce((acc, cur) => acc + cur.numShare, 0),
    };
  });
